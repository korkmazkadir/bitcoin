...
knit: (function(inputFile, encoding) { 
      workingDir <- '/home/kadir/Desktop/high_cc_32_64_96_128_160/1000KB';
...
### Background
In this report we have compared Bitcoin++ with Bitcoin Reduced Difficulty.

### Configuration
We have deployed 1005 nodes per each experiments.
We deployed the nodes on 15 machines from G5K. We cap the bandwidth of each process (20Mbps), and we have added latency to communication links (50ms).

For Bitcon++ experiments we have collected 30 rounds of data.
For Bitcoin experiments we collected 380, 240, and 120 rounds of data to match the number of disseminated block with corresponding Bitcoin++ experiment.

### Latency and Throughput measurements

Each node locally measures the round duration, and we have collected these measurements to create the latency plot presented here.
For the throughput, at the end of the experiment, each node calculates the throughput by dividing the size of appended data with total elapsed time.

### Forks
We have measured the average number of disseminated blocks per round for each experiment.
